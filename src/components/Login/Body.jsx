...
const Body = () => {
  const { handleLogin, error, success, isLoading } = useLogin();
  ...
  return (
    <div id="Body_1" className="min-h-screen bg-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div id="Body_2" className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-lg">
        ...
        {error && <div id="Body_6" className="mt-4 text-center text-red-600">{error}</div>}
        {success && <div id="Body_7" className="mt-4 text-center text-green-600">{success}</div>}
      </div>
    </div>
  );
};
